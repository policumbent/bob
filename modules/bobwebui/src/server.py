...
def get_main():
    ...
    for service in config.sections():
        ...
        service_enabled = get_service_action(service, 'enabled')
        ...
        enabled = True if service_enabled == 'enabled' else False
...
def get_service_journal_page(service):
    if service in config.sections():
        ...
        return template('journal', hostname=gethostname(), service=config.get(service, 'title'),
...
@route('/<service>/<action>/')
@route('/<service>/<action>/<lines:int>')
@route('/<service>/<action>/<lines:int>/')
# @auth_basic(login)
def get_service_action(service, action, lines=100):
    if service in config.sections():
        ...
        if action == 'start':
            ...
            return {action: 'OK'} if sdbus.enable_unit(unit) else {action: 'Fail'}
        elif action == 'disable':
            return {action: 'OK'} if sdbus.disable_unit(unit) else {action: 'Fail'}
            ...
            return get_service_journal(service, lines)
        elif action == 'enabled':
            return sdbus.get_unit_enable_state(unit)
        else:
            response.status = 400
